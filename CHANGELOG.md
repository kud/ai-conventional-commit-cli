...
- **New Codex CLI provider.** Generate commit messages via OpenAI Codex (`codex/<model>`, default `codex/gpt-5.5`), authenticated through your existing ChatGPT plan — no API key required, since it shells out to `codex exec` under the hood. ([fb70158](https://github.com/kud/ai-conventional-commit-cli/commit/fb70158fd1beffa258e9f5002aff357d7bf8fd77))
- **Commit bodies are no longer optional for real changes.** The shared generation prompt now requires a body for non-trivial diffs (feat/fix/refactor/multi-file), so every provider — not just Codex — reliably writes a description instead of a bare subject line. ([fb70158](https://github.com/kud/ai-conventional-commit-cli/commit/fb70158fd1beffa258e9f5002aff357d7bf8fd77))

### Documentation

- README restructured to match the canonical kud-site layout, bringing it in line with docs across other kud projects. ([19afd15](https://github.com/kud/ai-conventional-commit-cli/commit/19afd155384e0f7cbc6acd2b53af792ba3d6d185))
...
- The README is now a slim front page, with the full documentation split into multi-page docs hosted on kud.io. ([e5aef1b](https://github.com/kud/ai-conventional-commit-cli/commit/e5aef1b), [de50d30](https://github.com/kud/ai-conventional-commit-cli/commit/de50d30), [ce6cd21](https://github.com/kud/ai-conventional-commit-cli/commit/ce6cd21), [4e7e369](https://github.com/kud/ai-conventional-commit-cli/commit/4e7e369))

<details>
<summary>Internal (1 commit)</summary>

- Removed the old GitHub Pages site now that docs live on kud.io. ([160d8d7](https://github.com/kud/ai-conventional-commit-cli/commit/160d8d7))
...
Version bump only — no functional changes. Tagged the same day as 3.6.2.
...
- **Model strings are validated before a run starts.** A malformed `--model` value now fails immediately with a clear message rather than surfacing as an obscure provider error mid-generation. The opencode server startup timeout also became configurable via `AICC_OPENCODE_STARTUP_TIMEOUT_MS`, for slower machines where the default 30s was too tight. ([047d6f6](https://github.com/kud/ai-conventional-commit-cli/commit/047d6f6))
- Default model updated to `claude-sonnet-4.6`. ([c9dfe69](https://github.com/kud/ai-conventional-commit-cli/commit/c9dfe69))
...
- **Config moved to `~/.config/aicc` and `~/.cache/aicc`.** Fixes a crash in git worktrees where `.git` is a file rather than a directory; the old paths still work with a deprecation notice until removed in a future version. ([7575d0f](https://github.com/kud/ai-conventional-commit-cli/commit/7575d0f))
...
- Added a feature-highlights section to the README and dropped the stale mock dev-run section. ([d4a6017](https://github.com/kud/ai-conventional-commit-cli/commit/d4a6017), [f32dba8](https://github.com/kud/ai-conventional-commit-cli/commit/f32dba8))
...
- Increased the project icon display size across the README and docs site. ([dd43874](https://github.com/kud/ai-conventional-commit-cli/commit/dd43874))
...
- Added the project icon to the README and docs site. ([02d1b17](https://github.com/kud/ai-conventional-commit-cli/commit/02d1b17))
...
- `reword` now prints the updated commit once it has been applied, so you can confirm the result without a follow-up `git log`. ([93eeabd](https://github.com/kud/ai-conventional-commit-cli/commit/93eeabd))
...
- Documented the `--style` flag and its config-file equivalent. ([268d0fc](https://github.com/kud/ai-conventional-commit-cli/commit/268d0fc))
